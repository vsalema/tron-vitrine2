...
// =====================================================
// 🎞️ VITRINE - BEST VALUE (UI + logique)
// =====================================================


// DOM refs vitrine
const filmsActionsRow = document.getElementById('filmsActionsRow');
...
function __isFilmTab(){
  return getActiveTabKey() === 'channels';
}
...
function updateShowcaseButtonVisibility(){
  ...
  const isFilmTab = __isFilmTab();
  if(!isFilmTab){
    showcaseContainer.classList.add('hidden');
    if (filmsActionsRow) filmsActionsRow.classList.add('hidden');
    if(showcaseOpen) closeShowcase();
    return;
  }

  if (filmsActionsRow) filmsActionsRow.classList.remove('hidden');
  showcaseContainer.classList.remove('hidden');
}

function __recentIdSet(){
  const set = new Set();
  if(Array.isArray(newAdditionsIds)){
    for(const id of newAdditionsIds){
...
function __inferFlags(entry){
  const name = String(entry?.name || '');
  const u = name.toUpperCase();

  const is4k = /\b(4K|UHD)\b/i.test(name);
  const isVost = /\bVOST\b|\bVOSTFR\b|VOST-FR/i.test(u);
  // VF: on exclut VOSTFR pour eviter double
  const isVf = !isVost && (/\bVF\b|\bFRENCH\b|\bFR\b/.test(u));

  return { is4k, isVf, isVost };
}
...
function __passesShowcase(entry, recentSet){
  if(!entry) return false;

  // search
  if(showcaseSearch){
    const q = __normText(showcaseSearch);
    const n = __normText(entry.name);
    const g = __normText(entry.group);
    if(!n.includes(q) && !g.includes(q)) return false;
  }

  const flags = __inferFlags(entry);
  const tvgId = String(entry.tvgId || '').trim();
  const isRecent = tvgId && recentSet.has(tvgId);

  if(showcaseFilter === 'recent') return isRecent;
  if(showcaseFilter === 'fav') return !!entry.isFavorite;
  if(showcaseFilter === '4k') return flags.is4k;
  if(showcaseFilter === 'vf') return flags.isVf;
  if(showcaseFilter === 'vostfr') return flags.isVost;

  return true; // all
}

function __chipLabel(key, count){
  if(key === 'all') return `Tous (${count})`;
  if(key === 'recent') return `Recents (${count})`;
  if(key === 'fav') return `Favoris (${count})`;
  if(key === '4k') return `4K (${count})`;
  if(key === 'vf') return `VF (${count})`;
  if(key === 'vostfr') return `VOSTFR (${count})`;
  return `${key} (${count})`;
}

function __countForFilter(key, recentSet){
  // counts are affected by search (so chips reflect what you typed)
  let c = 0;
  for(const e of channels){
    if(!e) continue;
    const prev = showcaseFilter;
    showcaseFilter = key;
    const ok = __passesShowcase(e, recentSet);
    showcaseFilter = prev;
    if(ok) c += 1;
  }
  return c;
...
function __makeChip(key, label){
  const btn = document.createElement('button');
  btn.className = 'showcase-chip';
  btn.type = 'button';
  btn.textContent = label;
  btn.dataset.active = (showcaseFilter === key) ? 'true' : 'false';
...
function __makeBadge(text, cls=''){
  const s = document.createElement('span');
  s.className = 'showcase-badge' + (cls ? ' ' + cls : '');
  s.textContent = text;
  return s;
}

function __scrollRowByPage(rowEl, dir){
  if(!rowEl) return;
  const amount = Math.max(240, Math.round(rowEl.clientWidth * 0.85));
...
function renderShowcase(){
  ...
  const recentSet = __recentIdSet();

  // chips
  if(showcaseChips){
    showcaseChips.innerHTML = '';
    const keys = ['all','recent','fav','4k','vf','vostfr'];
    for(const k of keys){
      const count = __countForFilter(k, recentSet);
      const chip = __makeChip(k, __chipLabel(k, count));
      showcaseChips.appendChild(chip);
    }
  }
  ...
  for(let i=0;i<channels.length;i++){
    const e = channels[i];
    if(!e) continue;
    // keep index for playback
    e.__showcaseIndex = i;

    if(!__passesShowcase(e, recentSet)) continue;

    const grp = String(e.group || 'Autres').trim() || 'Autres';
    if(!grouped.has(grp)) grouped.set(grp, []);
    grouped.get(grp).push(e);
  }

  if(grouped.size === 0){
    const empty = document.createElement('div');
    empty.className = 'showcase-empty';
    empty.textContent = 'Aucun film ne correspond a ta recherche / filtre.';
  ...
  for(const [grp, items] of groups){
    ...
    navL.className = 'showcase-row-nav left';
    navL.type = 'button';
    navL.textContent = '◀';

    const navR = document.createElement('button');
    navR.className = 'showcase-row-nav right';
    navR.type = 'button';
    navR.textContent = '▶';
    ...
    navL.addEventListener('click', (ev) => { ev.stopPropagation(); __scrollRowByPage(row, -1); });
    navR.addEventListener('click', (ev) => { ev.stopPropagation(); __scrollRowByPage(row, 1); });

    for(const e of items){
      const card = document.createElement('div');
      card.className = 'showcase-card';
      card.tabIndex = 0;
      card.dataset.idx = String(e.__showcaseIndex);

      const poster = document.createElement('div');
      poster.className = 'showcase-poster';

      const logo = normalizeLogo(e.logo, e.name);
      if(logo && logo.type === 'image'){
        ...
        letter.textContent = (logo && logo.value) ? logo.value : (deriveLogoFromName(e.name).value);
      ...
      const tvgId = String(e.tvgId || '').trim();
      if(tvgId && recentSet.has(tvgId)) badges.appendChild(__makeBadge('NEW', 'new'));

      const flags = __inferFlags(e);
      if(flags.is4k) badges.appendChild(__makeBadge('4K'));
      if(flags.isVf) badges.appendChild(__makeBadge('VF'));
      if(flags.isVost) badges.appendChild(__makeBadge('VOST'));

      const fav = document.createElement('button');
      fav.className = 'showcase-fav';
      fav.type = 'button';
      fav.textContent = '★';
      fav.dataset.fav = e.isFavorite ? 'true' : 'false';
      fav.title = 'Ajouter / enlever des favoris';
      fav.addEventListener('click', (ev) => {
        ...
        renderFavoritesList();
        // refresh chips/counts (ex: Favoris)
        renderShowcase();
      });
      ...
      name.textContent = normalizeName(e.name);
      try { name.title = String(e.name || ''); } catch {}
...
if (showcaseOverlay) {
  ...
  showcaseOverlay.addEventListener('click', (ev) => {
    if (ev.target === showcaseOverlay) closeShowcase();
  });
}

if (showcaseSearchInput) {
  showcaseSearchInput.addEventListener('input', () => {
    showcaseSearch = showcaseSearchInput.value || '';
    renderShowcase();
  });
}

if (showcaseSearchClear) {
  showcaseSearchClear.addEventListener('click', () => {
    showcaseSearch = '';
...
window.addEventListener('keydown', (ev) => {
  if (ev.key === 'Escape' && showcaseOpen) closeShowcase();
});

// Switch tabs: hide / show button + close overlay if leaving films tab
document.addEventListener('click', (e) => {
  if (e.target && e.target.closest && e.target.closest('.tab-btn')) {
    setTimeout(() => {
...
// Initial visibility (after first render)
setTimeout(() => {
  updateShowcaseButtonVisibility();
}, 0);
