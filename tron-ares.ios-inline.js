...
const MAIN_PLAYLIST_URL = "https://raw.githubusercontent.com/vsalema/tvpt4/refs/heads/main/css/playlist-tron_tmdb_ranged-1.fixed.sorted.m3u";
...
async function openTmdbSynopsisBackdrop(entry, index){
  ...
  if(!data.ok){
    ...
    if(mEl) mEl.textContent = [y ? ('Année ' + y) : '', runtime ? runtime : '', rating !== null ? ('★ ' + rating + '/10') : '', (entry?.group || '')].filter(Boolean).join(' • ');
