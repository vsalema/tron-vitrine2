...
// =====================================================
// 🎞️ VITRINE (BEST VALUE) — overlay + recherche + chips + flèches + favoris
// =====================================================
...
const showcaseClearSearchBtn = document.getElementById('showcaseClearSearchBtn');
const showcaseChipsEl = document.getElementById('showcaseChips');

let showcaseOpen = false;
let showcaseFilterKey = 'all';
let showcaseQuery = '';

const SHOWCASE_FAV_KEY = 'tronAresShowcaseFav';
let showcaseFavSet = new Set();
try {
  const saved = JSON.parse(localStorage.getItem(SHOWCASE_FAV_KEY) || '[]');
  if (Array.isArray(saved)) showcaseFavSet = new Set(saved.map(String));
} catch {}

function showcaseEntryId(entry){
  const id = (entry?.tvgId || entry?.url || entry?.name || '').toString().trim();
  return id || (entry?.name || '').toString().trim();
}

function isFavEntry(entry){
  return showcaseFavSet.has(showcaseEntryId(entry));
}

function toggleFavEntry(entry){
  const id = showcaseEntryId(entry);
  if (!id) return;
  if (showcaseFavSet.has(id)) showcaseFavSet.delete(id);
  else showcaseFavSet.add(id);
  try { localStorage.setItem(SHOWCASE_FAV_KEY, JSON.stringify(Array.from(showcaseFavSet))); } catch {}
}

function isRecentEntry(entry){
  const id = (entry?.tvgId || '').toString().trim();
  return !!id && Array.isArray(newAdditionsIds) && newAdditionsIds.includes(id);
}

function is4KEntry(entry){
  const n = (entry?.name || '').toString();
  return /(\b4k\b|\buhd\b)/i.test(n);
}
function isVfEntry(entry){
  const n = (entry?.name || '').toString();
  if (/\bVOSTFR\b/i.test(n)) return false;
  return /\bVF\b/i.test(n);
}
function isVostfrEntry(entry){
  const n = (entry?.name || '').toString();
  return /\bVOSTFR\b/i.test(n);
}
...
function updateShowcaseButtonVisibility(){
  ...
  const hasItems = Array.isArray(channels) && channels.length > 0;
  showcaseBtn.disabled = !hasItems;
  showcaseBtn.style.opacity = hasItems ? '1' : '0.85';
}
...
function openShowcase(){
  ...
  if(showcaseSearchInput) showcaseSearchInput.focus();
}
...
function applyShowcaseFilters(items){
  let out = Array.isArray(items) ? items.slice() : [];

  // filtre chips
  if (showcaseFilterKey === 'recent') out = out.filter(isRecentEntry);
  else if (showcaseFilterKey === 'fav') out = out.filter(isFavEntry);
  else if (showcaseFilterKey === '4k') out = out.filter(is4KEntry);
  else if (showcaseFilterKey === 'vf') out = out.filter(isVfEntry);
  else if (showcaseFilterKey === 'vostfr') out = out.filter(isVostfrEntry);

  // recherche
  const q = (showcaseQuery || '').trim().toLowerCase();
  if (q) {
    out = out.filter(e => ((e?.name || '').toString().toLowerCase().includes(q)));
  }

  return out;
}

function computeChipCounts(baseItems){
  const items = Array.isArray(baseItems) ? baseItems : [];
  const counts = {
    all: items.length,
    recent: items.filter(isRecentEntry).length,
    fav: items.filter(isFavEntry).length,
    '4k': items.filter(is4KEntry).length,
    vf: items.filter(isVfEntry).length,
    vostfr: items.filter(isVostfrEntry).length,
  };
  return counts;
}

function renderChips(baseItems){
  if(!showcaseChipsEl) return;
  const counts = computeChipCounts(baseItems);
  ...
  showcaseChipsEl.innerHTML = '';

  for (const c of chips) {
    const b = document.createElement('button');
    b.className = 'showcase-chip' + (showcaseFilterKey === c.key ? ' active' : '');
    b.type = 'button';
    b.dataset.key = c.key;
    b.innerHTML = `<span>${c.label}</span><span class="showcase-chip-count">${counts[c.key] || 0}</span>`;
    b.addEventListener('click', (e) => {
      e.preventDefault();
      showcaseFilterKey = c.key;
      buildShowcase();
    });
    showcaseChipsEl.appendChild(b);
  }
}

function makeRowNavBtn(dir){
  const b = document.createElement('button');
  b.type = 'button';
  b.className = 'showcase-row-nav ' + (dir === 'prev' ? 'prev' : 'next');
  b.textContent = (dir === 'prev') ? '◀' : '▶';
  b.setAttribute('aria-label', dir === 'prev' ? 'Précédent' : 'Suivant');
  return b;
}
...
function buildShowcase(){
  ...
  const filtered = applyShowcaseFilters(channels);

  if(showcaseSub){
    const parts = [];
    parts.push(total ? `${total} films` : '0 film');
    parts.push('catégories');
    if (showcaseFilterKey !== 'all') parts.push(`filtre: ${showcaseFilterKey}`);
    if ((showcaseQuery || '').trim()) parts.push(`recherche: “${(showcaseQuery || '').trim()}”`);
    showcaseSub.textContent = parts.join(' — ');
  }
  ...
  if(!total){
    ...
    empty.textContent = "Aucun film n'est chargé. Charge une playlist Films (M3U) et réessaie.";
  ...
  if(!filtered.length){
    const empty = document.createElement('div');
    empty.className = 'showcase-empty';
    empty.textContent = "Aucun résultat avec ces filtres.";
    showcaseBody.appendChild(empty);
    return;
  }
  ...
  for(let i=0;i<channels.length;i++){
    const ch = channels[i];
    if(!filtered.includes(ch)) continue;
    const g = (ch?.group || 'Autres').toString().trim() || 'Autres';
    if(!grouped.has(g)) grouped.set(g, []);
    grouped.get(g).push({ entry: ch, index: i });
  }
  ...
  for(const [groupName, items] of grouped.entries()){
    ...
    h.className = 'showcase-section-title';
    const hLeft = document.createElement('span');
    hLeft.textContent = groupName;
    const hRight = document.createElement('span');
    hRight.style.opacity = '.75';
    hRight.style.fontSize = '10px';
    hRight.textContent = String(items.length);
    h.appendChild(hLeft);
    h.appendChild(hRight);

    const wrap = document.createElement('div');
    wrap.className = 'showcase-row-wrap';

    const row = document.createElement('div');
    row.className = 'showcase-row';

    const prev = makeRowNavBtn('prev');
    const next = makeRowNavBtn('next');
    ...
    for(const it of items){
      ...
      card.setAttribute('aria-label', `Lire ${entry?.name || 'film'}`);
      ...
      const addBadge = (txt) => {
        ...
        b.className = 'showcase-badge';
        b.textContent = txt;
        badges.appendChild(b);
      };

      if (isRecentEntry(entry)) addBadge('NEW');
      if (is4KEntry(entry)) addBadge('4K');
      if (isVostfrEntry(entry)) addBadge('VOSTFR');
      else if (isVfEntry(entry)) addBadge('VF');

      const favBtn = document.createElement('button');
      favBtn.type = 'button';
      favBtn.className = 'showcase-fav' + (isFavEntry(entry) ? ' active' : '');
      favBtn.textContent = '★';
      favBtn.title = isFavEntry(entry) ? 'Retirer des favoris' : 'Ajouter aux favoris';
      favBtn.addEventListener('click', (e) => {
        e.preventDefault();
        e.stopPropagation();
        toggleFavEntry(entry);
        favBtn.classList.toggle('active', isFavEntry(entry));
        // met à jour les counts si on est sur Favoris
        if (showcaseFilterKey === 'fav') buildShowcase();
        else renderChips(channels);
      });

      const logo = entry?.logo;
      if(logo && logo.type === 'image' && typeof logo.value === 'string' && logo.value.trim()){
        ...
        letter.className = 'showcase-letter';
        const v = String(entry?.name || '?').trim().slice(0,1).toUpperCase();
      ...
      name.textContent = entry?.name || 'Sans titre';

      const cat = document.createElement('div');
      cat.className = 'showcase-cat';
      cat.textContent = groupName;
      ...
      card.addEventListener('click', (e) => { e.preventDefault(); launch(); });
...
(() => {
  ...
  if (showcaseCloseBtn) {
    showcaseCloseBtn.addEventListener('click', (e) => {
      e.preventDefault();
      closeShowcase();
    });
  }
  ...
  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape' && showcaseOpen) closeShowcase();
  });

  // Recherche
  if (showcaseSearchInput) {
    const syncClear = () => {
      const has = (showcaseSearchInput.value || '').length > 0;
      if (showcaseClearSearchBtn) showcaseClearSearchBtn.style.opacity = has ? '1' : '.55';
    };
    syncClear();
  ...
  if (showcaseClearSearchBtn && showcaseSearchInput) {
    showcaseClearSearchBtn.addEventListener('click', (e) => {
      e.preventDefault();
  ...
  // Changement d'onglet (clic) → masque/affiche bouton
  document.addEventListener('click', (e) => {
    if (e.target && e.target.closest && e.target.closest('.tab-btn')) {
...
document.querySelectorAll('.tab-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    try { updateShowcaseButtonVisibility(); } catch {}
    ...
    // 🎞️ Vitrine : bouton uniquement sur Films
    updateShowcaseButtonVisibility();
